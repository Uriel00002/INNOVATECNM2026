...
// Entrada escalonada de tarjetas al cargar y al entrar en pantalla
(() => {
  const cards = [...document.querySelectorAll('.section-nav-card, .sample-card, .activity-item')];
  if (!cards.length) return;
  cards.forEach((card, index) => {
    card.classList.add('fx-card');
    card.style.setProperty('--fx-delay', `${Math.min(index % 6, 5) * 95}ms`);
  });
  if (!('IntersectionObserver' in window)) {
    cards.forEach(card => card.classList.add('fx-visible'));
    return;
  }
